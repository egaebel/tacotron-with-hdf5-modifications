...
def train(model, config, num_steps=1000000):
    ...
    with tf.Session() as sess:
        ...
        # Added
        batch_inputs = data_input.build_dataset_with_hdf5(
            sess,
            os.path.join(config.data_path, "mels"),
            os.path.join(config.data_path, "stfts"),
            os.path.join(config.data_path, "texts"),
            os.path.join(config.data_path, "text_lens"),
            os.path.join(config.data_path, "speech_lens"))
        print("Build dataset!")
